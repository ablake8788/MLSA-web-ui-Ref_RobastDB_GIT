...
def load_config() -> ConfigParser:
    ...
    raw = (os.getenv("APP_INI") or "").strip()
    ini_path = Path(raw) if raw else Path(__file__).resolve().with_name(INI_DEFAULT_NAME)

    read_ok = cfg.read(str(ini_path), encoding="utf-8")
    if not read_ok:
